...
const BookPagePortraitInner = <T,>(
  ...
  const [destinationPage, setDestinationPage] = useState<T | null>(null);
  const setDestination = useCallback((page: T | null) => {
  ...
  const turnPage = useCallback(
    (id: 1 | -1) => {
      setIsAnimating(true);
      setDestinationPage(id > 0 ? (next?.left ?? null) : (prev?.left ?? null));
    ...
    [onFlipStart, onPageFlip, rotateYAsDeg, setIsAnimating, next, prev]
  );
          ...
          const destLeft =
            id > 0
              ...
              : id < 0
                ? (prev?.left ?? null)
                : null;
          runOnJS(setDestination)(destLeft);
  ...
  return (
    <Animated.View style={containerStyle}>
      {destinationPage != null && renderPage != null && (
        <View
